...
<div class="container-xl px-4 mt-4">
    <div class="card mb-4">
        <div class="card-header">{{ $t('reports') }}</div>
        <div class="card-body">
                ...
                <div class="dataTable-top">
                    ...
                    <div class="dataTable-search d-flex">
                        <div class="col-4 me-2">
                            <select v-model="table.status" class="form-control">
                ...
                <div class="dataTable-container">
                    <table id="datatablesSimple" class="dataTable-table">
                        <thead>
                            ...
                            <th>
                                <a href="#" class="dataTable-sorter">{{ $t('reason') }}</a>
                            </th>
                            <th>
                        ...
                        <tbody>
                            <tr v-for="(report,index) in items" :key="index">
                                <td><a :href="$router.resolve({name: 'site-blog-index', params: {id: report.blog.__id, post: report.blog.slug}}).href" target="_blank">{{ report.blog.title }}</a></td>
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="editModalLabel">{{ $t('edit_report') }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form v-if="edit">
        <div class="modal-body">
            <div class="row">
                <div v-if="edit.comment" class="col-md-4">
                    <h2 >{{ $t('comment:') }}</h2>
                    <div v-html="edit.comment.content"></div>
                    <div class="card-body text-center">
                        <img class="img-account-profile rounded-circle mb-2" :src="edit.comment.user.image">
                        <div class="small font-italic text-muted mb-4"></div>
                        <input disabled :value="Object.values(edit.comment.user.name).join(' ')" class="form-control">
                        <input disabled :value="edit.comment.user.username" class="form-control mt-3">
                        <input disabled :value="edit.comment.user.email" class="form-control mt-3">
                    </div>
                </div>
                <div v-else class="col-md-4">
                    <div class="card-body text-center">
                        <img class="img-account-profile rounded-circle mb-2" :src="edit.blog.user.image">
                        <div class="small font-italic text-muted mb-4"></div>
                        <input disabled :value="Object.values(edit.blog.user.name).join(' ')" class="form-control">
                        <input disabled :value="edit.blog.user.username" class="form-control mt-3">
                        <input disabled :value="edit.blog.user.email" class="form-control mt-3">
                    </div>
                </div>
                <div v-html="edit.blog.content" class="col-md-8"></div>
            </div>
        </div>
        <div class="modal-footer">
            <button @click="banAction" type="button" class="btn btn-danger">{{ $t('ban_user') }}</button>
            <button @click="deleteAction" type="button" class="btn btn-danger">{{ edit.comment ? $t('delete_comment'): $t('delete_blog') }}</button>
...
export default {
    ...
    methods: {
        async deleteAction(){
            adminService.reportActionDelete(this.edit)
            this.items.splice(this.edit.index,1)
        },
        async banAction(){
            adminService.reportActionBan(this.edit)
            this.items.splice(this.edit.index,1)
        },
        selectEdit(index){
        ...
        async reportDelete(index){
            const item = this.items[index]
            if(item.step){
                await adminService.reportDelete(item)
        ...
        async get(){
            const response = await adminService.reportList(this.table)
